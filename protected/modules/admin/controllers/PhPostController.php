...
class PhPostController extends Backend
{
	...
	*/
	public function actionCreate()
	{
		...
		if(isset($_POST['PhPost']))
		{
			$model->attributes=$_POST['PhPost'];
	...
	*/
	...
	public function actionBatch(){
		...
		switch ( $command ) {
			case 'delete':
			//删除
				PhPost::model()->deleteAll($criteria);
				break;
			case 'show':
			//显示
				PhPost::model()->updateAll(array('status' => 1), $criteria);
				break;
			case 'hide':
			//隐藏
				PhPost::model()->updateAll(array('status' => 0), $criteria);
				break;
			case 'sortOrder':
				$sortOrder = $_POST['order'];
				foreach((array)$ids as $id){
					$catalogModel = PhPost::model()->findByPk($id);
					if($catalogModel){
	 ...
	 * @return PhPost the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=PhPost::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param PhPost $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='ph-post-form')
		{
