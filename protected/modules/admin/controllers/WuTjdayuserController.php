...
class WuTjdayuserController extends Backend
{
	...
	*/
	public function actionCreate()
	{
		...
		if(isset($_POST['WuTjdayuser']))
		{
			$model->attributes=$_POST['WuTjdayuser'];
	...
	*/
	...
	public function actionBatch(){
		...
		switch ( $command ) {
			case 'delete':
			//删除
				WuTjdayuser::model()->deleteAll($criteria);
				break;
			case 'show':
			//显示
				WuTjdayuser::model()->updateAll(array('status' => 1), $criteria);
				break;
			case 'hide':
			//隐藏
				WuTjdayuser::model()->updateAll(array('status' => 0), $criteria);
				break;
			case 'sortOrder':
				$sortOrder = $_POST['order'];
				foreach((array)$ids as $id){
					$catalogModel = WuTjdayuser::model()->findByPk($id);
					if($catalogModel){
	 ...
	 * @return WuTjdayuser the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=WuTjdayuser::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param WuTjdayuser $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='wu-tjdayuser-form')
		{
