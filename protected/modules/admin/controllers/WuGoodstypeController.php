...
class WuGoodstypeController extends Backend
{
	...
	*/
	public function actionCreate()
	{
		...
		if(isset($_POST['WuGoodstype']))
		{
			$model->attributes=$_POST['WuGoodstype'];
			$model->goodsdate=time();
	...
	*/
	...
	public function actionBatch(){
		...
		switch ( $command ) {
			case 'delete':
			//删除
				WuGoodstype::model()->deleteAll($criteria);
				break;
			case 'show':
			//显示
				WuGoodstype::model()->updateAll(array('status' => 0), $criteria);
				break;
			case 'hide':
			//隐藏
				WuGoodstype::model()->updateAll(array('status' => 1), $criteria);
				break;
			case 'sortOrder':
				$sortOrder = $_POST['order'];
				foreach((array)$ids as $id){
					$catalogModel = WuGoodstype::model()->findByPk($id);
					if($catalogModel){
	 ...
	 * @return WuGoodstype the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=WuGoodstype::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param WuGoodstype $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='wu-goodstype-form')
		{
