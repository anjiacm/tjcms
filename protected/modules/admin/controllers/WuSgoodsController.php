...
class WuSgoodsController extends Backend
{
    ...
    public function init(){
        parent::init();
        $this->_goodstypelist=WuSgoods::getgoodslist();
    }
	...
	*/
	public function actionCreate()
	{
		...
		if(isset($_POST['WuSgoods']))
		{
			$model->attributes=$_POST['WuSgoods'];
			$model->s_gooddate=time();
	...
	*/
	...
	public function actionBatch(){
		...
		switch ( $command ) {
			case 'delete':
			//删除
				WuSgoods::model()->deleteAll($criteria);
				break;
			case 'show':
			//显示
				WuSgoods::model()->updateAll(array('status' => 0), $criteria);
				break;
			case 'hide':
			//隐藏
				WuSgoods::model()->updateAll(array('status' => 1), $criteria);
				break;
			case 'sortOrder':
				$sortOrder = $_POST['order'];
				foreach((array)$ids as $id){
					$catalogModel = WuSgoods::model()->findByPk($id);
					if($catalogModel){
	 ...
	 * @return WuSgoods the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=WuSgoods::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param WuSgoods $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='wu-sgoods-form')
		{
