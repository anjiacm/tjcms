...
class WuTjuseruphourController extends Backend
{
	...
	*/
	public function actionCreate()
	{
		...
		if(isset($_POST['WuTjuseruphour']))
		{
			$model->attributes=$_POST['WuTjuseruphour'];
	...
	*/
	...
	public function actionBatch(){
		...
		switch ( $command ) {
			case 'delete':
			//删除
				WuTjuseruphour::model()->deleteAll($criteria);
				break;
			case 'show':
			//显示
				WuTjuseruphour::model()->updateAll(array('status' => 1), $criteria);
				break;
			case 'hide':
			//隐藏
				WuTjuseruphour::model()->updateAll(array('status' => 0), $criteria);
				break;
			case 'sortOrder':
				$sortOrder = $_POST['order'];
				foreach((array)$ids as $id){
					$catalogModel = WuTjuseruphour::model()->findByPk($id);
					if($catalogModel){
	 ...
	 * @return WuTjuseruphour the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=WuTjuseruphour::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param WuTjuseruphour $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='wu-tjuseruphour-form')
		{
