...
class MacLoadingController extends Backend
{
	...
	*/
	public function actionCreate()
	{
		...
		if(isset($_POST['MacLoading']))
		{
			$model->attributes=$_POST['MacLoading'];
	...
	*/
	...
	public function actionBatch(){
		...
		switch ( $command ) {
			case 'delete':
			//删除
				MacLoading::model()->deleteAll($criteria);
				break;
			case 'show':
			//显示
				MacLoading::model()->updateAll(array('status' => 1), $criteria);
				break;
			case 'hide':
			//隐藏
				MacLoading::model()->updateAll(array('status' => 0), $criteria);
				break;
			case 'sortOrder':
				$sortOrder = $_POST['order'];
				foreach((array)$ids as $id){
					$catalogModel = MacLoading::model()->findByPk($id);
					if($catalogModel){
	...
	public function actionCreateCache()
	{
		...
		if($data){
			$this->message('success', 'create cache success');
		}else{
			$this->message('error', 'create cache fail');
		}
	}
	 ...
	 * @return MacLoading the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=MacLoading::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param MacLoading $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='mac-loading-form')
		{
