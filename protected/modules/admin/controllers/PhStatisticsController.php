...
class PhStatisticsController extends Backend
{
	...
	*/
	public function actionCreate()
	{
		...
		if(isset($_POST['PhStatistics']))
		{
			$model->attributes=$_POST['PhStatistics'];
			if($model->save())
	...
	*/
	...
	public function actionBatch(){
		...
		switch ( $command ) {
			case 'delete':
			//删除
				PhStatistics::model()->deleteAll($criteria);
				break;
			case 'show':
			//显示
				PhStatistics::model()->updateAll(array('status' => 1), $criteria);
				break;
			case 'hide':
			//隐藏
				PhStatistics::model()->updateAll(array('status' => 0), $criteria);
				break;
			case 'sortOrder':
				$sortOrder = $_POST['order'];
				foreach((array)$ids as $id){
					$catalogModel = PhStatistics::model()->findByPk($id);
					if($catalogModel){
	 ...
	 * @return PhStatistics the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=PhStatistics::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param PhStatistics $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='ph-statistics-form')
		{
