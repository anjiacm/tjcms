...
class WuWeekController extends Backend
{
	...
	*/
	public function actionCreate()
	{
		...
		if(isset($_POST['WuWeek']))
		{
			$model->attributes=$_POST['WuWeek'];
	...
	*/
	...
	public function actionBatch(){
		...
		switch ( $command ) {
			case 'delete':
			//删除
				WuWeek::model()->deleteAll($criteria);
				break;
			case 'show':
			//显示
				WuWeek::model()->updateAll(array('status' => 1), $criteria);
				break;
			case 'hide':
			//隐藏
				WuWeek::model()->updateAll(array('status' => 0), $criteria);
				break;
			case 'sortOrder':
				$sortOrder = $_POST['order'];
				foreach((array)$ids as $id){
					$catalogModel = WuWeek::model()->findByPk($id);
					if($catalogModel){
	 ...
	 * @return WuWeek the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=WuWeek::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param WuWeek $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='wu-week-form')
		{
