...
class PhApplicationController extends Backend
{
   ...
   //初始化函数
	...
	public function actionIndex(){
		...
		$osType = $model->getOsTypeArray();
		$this->render('index', array ('model' => $model, 'datalist' => $result , 'pagebar' => $pages,'osType'=>$osType));
	}
	...
	*/
	public function actionCreate()
	{
		...
		if(isset($_POST['PhApplication']))
		{
			$model->attributes=$_POST['PhApplication'];
			if($model->save())
	...
	*/
	...
	public function actionBatch(){
		...
		switch ( $command ) {
			case 'delete':
			//删除
				PhApplication::model()->deleteAll($criteria);
				break;
			case 'show':
			//显示
				PhApplication::model()->updateAll(array('status' => 1), $criteria);
				break;
			case 'hide':
			//隐藏
				PhApplication::model()->updateAll(array('status' => 0), $criteria);
				break;
			case 'sortOrder':
				$sortOrder = $_POST['order'];
				foreach((array)$ids as $id){
					$catalogModel = PhApplication::model()->findByPk($id);
					if($catalogModel){
	 ...
	 * @return PhApplication the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=PhApplication::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param PhApplication $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='ph-application-form')
		{
	...
	public function actionUploadSimple(){
		...
		if(Yii::app()->request->isPostRequest) {
			//普通上传
			$uploader->initSimple('appicon')->uploadSimple('simple_file');
