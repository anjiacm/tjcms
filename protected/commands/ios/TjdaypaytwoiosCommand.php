...
class TjdaypaytwoiosCommand  extends CConsoleCommand
{
    public $_statdate ;
    public $_enddate ;
    public $_date ;
    ...
    /*统计每日新增*/
    private function daypaytwo(){
        $tr = Yii::app()->db->beginTransaction();
        $daysql='INSERT INTO y_wu_tjselectall(dodate,channel,channel_t,channel_s,app_version,dem_num,pay_type,onedayuser) 
                    SELECT 
        ...
        if($dayinsert){
            ...
            $art['code'] =0;
            $art['msg'] ='写入成功';
            file_put_contents($this->_files.'/tongji/suc/'. $this->_date.'_tj.log','统计1天内付费(含支付渠道)——'.$dayinsert."\r\n",FILE_APPEND); //测试用
        }else{
            $tr->rollBack();
            $art['code'] =1;
            $art['msg'] ='写入失败';
            file_put_contents($this->_files.'/tongji/err/'. $this->_date.'_tj.log','统计1天内付费(含支付渠道)——'.$dayinsert.'-失败时间-'.date('Y-m-d H:i:s')."\r\n",FILE_APPEND); //测试用
        }
