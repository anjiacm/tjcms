...
{	
	 ...
	 * @return boolean
	 */
    public static function sendMail($title,$touseradd,$tocontenr) {

        $mail =new Semail();

        $mail->setServer("smtp.sina.cn", "18815239392@sina.cn", "02061226Wjl.", 465, true); //设置smtp服务器，到服务器的SSL连接
        $mail->setFrom("18815239392@sina.cn"); //设置发件人
        $mail->setReceiver($touseradd); //设置收件人，多个收件人，调用多次
        //$mail->setCc("XXXX"); //设置抄送，多个抄送，调用多次
        //$mail->setBcc("XXXXX"); //设置秘密抄送，多个秘密抄送，调用多次
// $mail->addAttachment("http://www.anjiacm.com/textdir/20160225/img/xiuyixiu.mp3"); //添加附件，多个附件，调用多次
        $mail->setMail($title, $tocontenr); //设置邮件主题、内容
       $resdo= $mail->sendMail(); //发送
        // return($mail->Send());


			// 添加邮件日志
			$model = new MailLog ();
			$model->accept = $touseradd;
			$model->subject = $title;
			$model->message = $tocontenr;
...
//	}
	...
	{
		if (empty($name)) {
			$fileName = dirname(Yii::app()->BasePath) . "/log/log_" . date('Ymd') . ".txt";
		} else {
			$fileName = dirname(Yii::app()->BasePath) . "/log/{$name}_" . date('Ymd') . ".txt";
		}
	...
	public static function writeHtml($content, $name = "",$dir = "html")
	{
		if (!file_exists($dir)){
			self::createDir($dir);
		}
		if (empty($name)) {
			$fileName = dirname(Yii::app()->BasePath) . "/{$dir}/{$dir}_" . date('Ymd') . ".html";
		} else {
			$fileName = dirname(Yii::app()->BasePath) . "/{$dir}/{$name}". ".html";
		}
		$fp = fopen($fileName, 'w');
		fwrite($fp, $content);
		fclose($fp);
	...
	//二位数组转为一位数组
	public static function arrayMap($data,$k,$v)
	{
		$newData=array();
		foreach($data as $key=>$val){
			$newData[$val[$k]]= $val[$v];
