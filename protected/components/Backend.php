...
//			'oAuth' => array('index','update','batch'),                                             //第三方登录管理
            'WuDayarpu' => array('index'),                                                //编辑器
            'WuTjuserall' => array('index', 'ztqs','kzz','kzqd', 'paychang','payindex','paymoney','qdlist','qudao','editindex'),
        );
		return $acl;
	}
		...
		if($groupid && $acl){
			...
			if($group->acl){
				if($groupid == User::AdminGroupID && $group->acl == 'Administrator'){
                    ...
                    $bool = stripos($group->acl, $acl) ? true: false;
				}			
			}
		}
